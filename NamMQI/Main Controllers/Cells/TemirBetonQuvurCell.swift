//
//  TemirBetonQuvurCell.swift
//  NamMQI
//
//  Created by Abdusamad Mamasoliyev on 18/04/24.
//
import UIKit
class TemirBetonQuvurCell: UITableViewCell {
    
    static let identifeir = "TemirBetonQuvurCell"
    ...
    let temirBetonPicker = UIPickerView()
    let temirBeton = ["500", "600", "700", "800", "900", "1000", "1200", "1400", "1600"]
    ...
    func setupPicker() {
        ...
        temirBetonPicker.delegate = self
        temirBetonPicker.dataSource = self
        ...
        ichkiDiametrTF.inputView = temirBetonPicker
    }
    
    @objc func donePress() {
        let row = temirBetonPicker.selectedRow(inComponent: 0)
        ichkiDiametrTF.text = temirBeton[row]
        ichkiDiametrTF.resignFirstResponder()
    }
}

extension TemirBetonQuvurCell {
